...
#define ESP32
// Factor dervied from empherical analysis.  The ESP32 screws with interrupt timing
#define ESP32_Factor 0.298
...
/*  Serial.print("compareValue="); Serial.println(compareValue);
  Serial.print("value="); Serial.println(value);
  Serial.print("Bottomvalue="); Serial.println((compareValue * (1.0 - VARYVALUE)));
  Serial.print("Topvalue="); Serial.println((compareValue * (1.0 + VARYVALUE)));
  */
...
void ICACHE_RAM_ATTR serviceInterruptAnem()
{
  ...
  //Serial.println("ESP32 Anem Interrupt");
...
//Serial.println("---------------------------------------_____--------------In Rain Interrupt");
...
SDL_Weather_80422::SDL_Weather_80422(int pinAnem, int pinRain, int intAnem, int intRain, int ADChannel, int ADMode)
{
  ...
  if (_ADMode == SDL_MODE_INTERNAL_AD)
  {
#ifdef ESP32
    Serial.print("ADChannel=");
    Serial.println(ADChannel);

    pinMode(ADChannel, INPUT);
    adcAttachPin(ADChannel);
...
#ifdef ESP32
  returnTime =  ((1 / (time)) * WIND_FACTOR ) * ESP32_Factor;
#else
...
float A2D(float SenVal)
{
  ...
  else if (SenVal > 4094) return (4094);
...
float SDL_Weather_80422::current_wind_direction()
{
  ...
  else
  {
    ...
    // we need to average signals. - ESP32 does not have a good ADC
...
#define NUMSAMPLES 10
    ...
    {
      rawValue = analogRead(_ADChannel);

      totalValue = totalValue + rawValue;
    }

    finalValue = totalValue / NUMSAMPLES;





    voltageValue = A2D(finalValue) - 0.02;
...
float SDL_Weather_80422::current_wind_direction_voltage()
{
  ...
  {
    // use internal A/D converter
    int rawValue = analogRead(_ADChannel);
    //voltageValue = (analogRead(_ADChannel) / 4096.0) * Vcc;
    voltageValue = A2D(rawValue);
...
#ifdef ESP32
    _currentWindSpeed = (((float)_currentWindCount / (_timeSpan)) * WIND_FACTOR * 1000000) * ESP32_Factor;
#else
